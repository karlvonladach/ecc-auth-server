var ripemd160 = (function () {
  ...
  function MDfinish (MDbuf, data, lswlen, mswlen)
  {
    var X = new Array(16);
    zeroX(X);
  ...
  {
    var tmp  = (fourChars[3] & 255) << 24;
    tmp   |= (fourChars[2] & 255) << 16;
    tmp   |= (fourChars[1] & 255) << 8;
    tmp   |= (fourChars[0] & 255);  
  ...
  function RMD(message)
  {
    ...
    var array8 = new Uint8Array(message);
    //var array32 = new Uint32Array(message);
    length = array8.byteLength;
    ...
    for (var nbytes=length; nbytes > 63; nbytes -= 64)
    {
      for (var i=0; i < 16; i++)
      {
        X[i] = BYTES_TO_DWORD(array8.slice(j, 4));
        j += 4;
      }
      compress(MDbuf, X);
    }
  
    MDfinish(MDbuf, array8.slice(j), length, 0);
